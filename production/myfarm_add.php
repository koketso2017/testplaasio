...
<?php
  ...
  if(isset($_POST['savecat']))
   { 
    ...
    $sql1 = "INSERT into plaas_data_farm(uniqueid, farmname, category, location, village, country, areasize, userid, description) VALUES (?,?,?,?,?,?,?,?,?)";   
    $q1 = $pdo->prepare($sql1);
    $q1->execute(array($uniqueid,$farmname,$categoryid,$farmlocation,$farmvillage,$farmcountry,$farmarea,$userid,$description));      
    echo '<meta content="2;myfarms.php?farm_add_sucess" http-equiv="refresh" />';// redirects user view page after 3 seconds.
    MyData::disconnect();  
    } 
     else
      {        
      echo '<meta content="2;myfarm_add.php?farm_add_error" http-equiv="refresh" />';// redirects user view page after 3 seconds. 
      }
   }
?>   
