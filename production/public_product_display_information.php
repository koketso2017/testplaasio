<?php
  ...
  if(isset($_GET['id']) && $_GET['id'] != '' && $_GET['id'] > 0){
   $pid = null;
    ...
    $q = $pdo->prepare($sql);
    $q->execute(array($pid));   
    echo '<meta content="0;public_product_display_information.php?productid='. $ppid .'&productIuniqueid='. $prproductIuniqueid .'&gallery_sucess_delete" http-equiv="refresh" />';
    MyData::disconnect(); 
} 
...
if (isset($_POST["sub1"])) {
  ...
  if (count($_FILES["user_files"]) > 0) {
    ...
    echo '<meta content="1;public_product_display_information.php?productid='. $productid .'&productIuniqueid='. $productIuniqueid .'" http-equiv="refresh" />';
...
if ( $productid=='') {
    echo '<meta content="2;public_products_display.php?error_operation" http-equiv="refresh" />';// redirects user view page after 3 seconds. 
  }
                    ...
                    <div class="col-md-5 col-sm-5 col-xs-12" style="border:0px solid #e5e5e5; color: #000;">
                      ...
                      <p style="font-weight: bold;"><i class="fa fa-crosshairs"></i> NO Of Young Ones: <?php echo !empty($contkids)?$contkids:''; ?></p>
                       <?php
                        ...
                        <?php echo '<a href="public_product_display_information.php?productid='. $row['id'].'&productIuniqueid='.$row['uniqueid'].'">'?><p><i class="fa fa-check-square" style="font-size: 100%; color: #2B1B17;"> </i> </i> <?php echo !empty($mentryuniqueid)?$mentryuniqueid:'';?></p><?php echo '</a>'?>
                        <?php
                        }
                       }
                        MyData::disconnect();
...
<script type="text/javascript">
  ...
  if(iAnswer){
    window.location = 'public_product_display_information.php?id=' + Id;
  }
  }
