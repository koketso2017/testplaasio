<?php
  ...
  if ( null==$animalid ) {
   echo '<meta content="2;animal_breed_cross_list.php?animal_idupd_error" http-equiv="refresh" />';// redirects user view page after 3 seconds. 
  } 

    $pdo = MyData::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "SELECT * FROM `plaas_cross_breeds` WHERE id = '$animalid'";
    $q = $pdo->prepare($sql);
...
<?php
  ...
  if(isset($_POST['savecat']))
   { 
    ...
    $pdo = MyData::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql1 = "UPDATE plaas_cross_breeds SET name = ?, categoryid = ?, cross_a = ?, cross_b = ?, productiontype = ?, countryoforigin = ?, description = ?, icon = ?, modified = ? WHERE id = ?";   
    $q1 = $pdo->prepare($sql1);
    $q1->execute(array($breedname,$breedcategory,$breedcrossa,$breedcrossb,$breedproduct,$breedcountry,$breedescription,$breedicon,$lastmod,$animalid));    
    echo '<meta content="2;main_animal_cross_edit.php?animal_id='. $animalid .'&breed_sucess_update" http-equiv="refresh" />';// redirects user view page after 3 seconds.
    MyData::disconnect(); 
   }
?>   
