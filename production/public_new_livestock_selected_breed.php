<?php
  ...
  if ( null==$mypurebreedid ) {
   echo '<meta content="2;public_pure_breeds_display.php?selection_id_error" http-equiv="refresh" />';// redirects user view page after 3 seconds. 
  }
          ...
          <div class="">
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2><?php echo !empty($categonamea)?$categonamea:''; ?> <small> <a href="public_pure_breeds_display.php">Livestock selection</a> <i class="fa fa-chevron-right"></i> <?php echo '<a href="public_pure_breeds_display_by_category.php?categoryname='. $categoname .'&user_key=<?php echo $userhashid?>">'?> <?php echo !empty($categonamea)?$categonamea:''; ?> breeds selection <?php echo '</a>'?> <i class="fa fa-chevron-right"></i> <b><a href="#"><?php echo !empty($bredname)?$bredname:''; ?></a></b></small></h2>
                    ...
                    <form class="form-horizontal form-label-left input_mask" method="POST">  
                      ...
                      <div class="col-md-9 col-sm-9 col-xs-12 form-group has-feedback" >  
                       <?php  
                        ...
                        $result= $con->query($query);
                       ?>    
                       <select data-toggle="tooltip" title="Reference ID" style="width: 100%;" class="form-control" name="userid" onChange="getUser(this.value)" id="inputEmail3">
                        <option value="">Initialise reference</option>
                        <?php while ($row=$result->fetch_array(MYSQLI_ASSOC)) { ?>
...
<?php
  ...
  if(isset($_POST['savecat']))
   { 
    ...
    echo '<meta content="2;public_new_livestock_selected_breed_redirect.php?pname='. $dispcategorname .'&pcatego='. $dispcatetype .'&ptotal='. $livetotal .'&userid='. $liveuser .'" http-equiv="refresh" />';// redirects user view page after 3 seconds.
    MyData::disconnect();  
    } 
     else
      {        
      echo '<meta content="2;public_products_display.php?livestock_add_error" http-equiv="refresh" />';// redirects user view page after 3 seconds. 
      }
   }
?>    