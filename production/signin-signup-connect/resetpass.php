...
if(isset($_GET['id']) && isset($_GET['code']))
{
	...
	$stmt = $user->runQuery("SELECT * FROM property_spy_users WHERE userID=:uid AND tokenCode=:token");
	...
	if($stmt->rowCount() == 1)
	{
		if(isset($_POST['btn-reset-pass']))
		{
			...
			{
				$password = md5($cpass);
				$stmt = $user->runQuery("UPDATE property_spy_users SET userPass=:upass WHERE userID=:uid");
...
<html>
  <head>
    <title>Password Reset | AparLink</title>
    <!-- Bootstrap -->
