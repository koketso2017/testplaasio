            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
              <div class="dashboard_graph">
                ...
                <div class="col-md-3 col-sm-3 col-xs-12 bg-white">
                  <div class="x_title">
                    <h2 style="font-size: 130%;">System Livestock statistics</h2>
                    <div class="clearfix"></div>
                  </div>

                  <div class="col-md-12 col-sm-12 col-xs-6">
                  <?php
                   if ($result0101 = mysqli_query($con, "SELECT * FROM plaas_breeds ORDEr by RAND() LIMIT 5")) {
 
                    while ($row = mysqli_fetch_assoc($result0101))
                     { 
                      $selectedbreed =  $row['countryoforigin'];
                      $selectednamebreed =  $row['name'];
                      $selectednamebreedid =  $row['id'];
                      $selectednamebreedct =  $row['categoryid'];
             
                      $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                      $query_ams_settings = "SELECT * FROM plaas_countries WHERE id = '$selectedbreed'";
                      $q = $pdo->prepare($query_ams_settings);
                      $q->execute(array());
                      $data = $q->fetch(PDO::FETCH_ASSOC); 
                      $dispcontid = $data['name'];
                      $dispcountid = $data['id'];
             
                      $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                      $query_ams_settings = "SELECT * FROM categories WHERE id = '$selectednamebreedct'";
                      $q = $pdo->prepare($query_ams_settings);
                      $q->execute(array());
                      $data = $q->fetch(PDO::FETCH_ASSOC); 
                      $dispctname = $data['name'];
                      
                      if ($result0133 = mysqli_query($con, "SELECT sum(total) as suptotal FROM products WHERE firstbreed = '$selectednamebreedid'")) {
 
                        while ($row = mysqli_fetch_assoc($result0133))
                        { 
                         $supprocducts =  $row['suptotal'];
                        }
                        /* close result set */
                        mysqli_free_result($result0133);
                       }
                     ?>
                    <div>
                      <p><?php echo !empty($selectednamebreed)?$selectednamebreed:''; ?>: <?php echo !empty($dispctname)?$dispctname:''; ?></p>
                      <div class="">
                        <div class="progress progress_sm" style="width: 76%; color: #348017;">
                          <div class="progress-bar bg-green" role="progressbar" data-toggle="tooltip" title="<?php echo !empty($supprocducts)?$supprocducts:''; ?> <?php echo !empty($selectednamebreed)?$selectednamebreed:''; ?> <?php echo !empty($dispctname)?$dispctname:''; ?>" data-transitiongoal="<?php echo !empty($supprocducts)?$supprocducts:''; ?>"></div>
                        </div>
                      </div>
                    </div>
                        <?php
...
<?php
    ...
    $strQuery = "SELECT name, total FROM plaas_products_statistics_user WHERE userid = '$userid' ORDER BY total DESC LIMIT 5";
...
/*Create an object for the column chart using the FusionCharts PHP class constructor. Syntax for the constructor is ` FusionCharts("type of chart", "unique chart id", width of the chart, height of the chart, "div id to render the chart", "data format", "data source")`. Because we are using JSON data to render the chart, the data format will be `json`. The variable `$jsonEncodeData` holds all the JSON data for the chart, and will be passed as the value for the data source parameter of the constructor.*/
        ...
        $columnChart = new FusionCharts("column2D", "myFirstChart" , 300, 255, "chart-1", "json", $jsonEncodedData);
