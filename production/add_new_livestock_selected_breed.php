<?php
  ...
  $mypurebreedid = null;
  if ( !empty($_GET['livestock_breed_category_id'])) {
    $mypurebreedid = $_REQUEST['livestock_breed_category_id'];
  } 
          ...
          <div class="">
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Animals <small> <a href="add_new_livestock.php">Livestock selection</a> <i class="fa fa-chevron-right"></i> <?php echo '<a href="add_new_livestock_pure_breed.php?livestock_category_id='. $categoname .'&selected_product='. $categonamea .'">'?> <?php echo !empty($categonamea)?$categonamea:''; ?> breeds selection <?php echo '</a>'?> <i class="fa fa-chevron-right"></i> <b><a href="#"><?php echo !empty($bredname)?$bredname:''; ?></a></b></small></h2>
                    <ul class="nav navbar-right panel_toolbox">
                      <li><?php echo '<a href="my_livestock_listing.php">'?><button type="button" class="btn btn-dark"><i class="fa fa-list"></i></button><?php echo '</a>'?></li>
                  ...
                  <div class="x_content">
                    <form class="form-horizontal form-label-left input_mask" method="POST">  
                      ...
                      <div class="col-md-9 col-sm-9 col-xs-12 form-group has-feedback">
                        <input required="true" data-toggle="tooltip" title="Tag or branding identification number attached the product" type="text" style="width: 100%;" name="prtid" class="form-control has-feedback-left" id="inputSuccess2" placeholder="Tag or branding identification number attached the product" value="<?php echo !empty($livetaggaingsel)?$livetaggaingsel:''; ?>">
                      ...
                      <div class="col-md-9 col-sm-9 col-xs-12 form-group has-feedback">
                        <input required="true" data-toggle="tooltip" title="Product color" type="text" style="width: 100%;" name="prcol" class="form-control has-feedback-left" id="inputSuccess2" placeholder="Product color" value="">
                      ...
                      <div class="col-md-9 col-sm-9 col-xs-12 form-group has-feedback" > 
                       ...
                       ?>    
                       <select data-toggle="tooltip" title="Maturity selection" style="width: 100%;" required="true" class="form-control" name="prsex" id="inputEmail3">
...
<?php
  ...
  if(isset($_POST['savecat']))
   { 
    ...
    $pdo = MyData::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); 
    $sql1 = "INSERT into products(uniqueid, chipid, tagid, color, sex, image, categoryid, userid, farmID, firstbreed, secondbreed, lastbreedid, total, upsign, productsize) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";   
    $q1 = $pdo->prepare($sql1);
    $q1->execute(array($liveuniqueid,$livechip,$livetag,$livecolor,$livesex,$livepic,$categoname,$userid,$livefarmid,$mypurebreedid,$livesbreed,$livetbreed,$livetotal,$livesign, $productsize));    
    echo '<meta content="2;add_new_livestock_selected_breed_redirect.php?pname='. $dispcategorname .'&pcatego='. $dispcatetype .'&ptotal='. $livetotal .'&userid='. $userid .'" http-equiv="refresh" />';// redirects user view page after 3 seconds.
    MyData::disconnect();  
    } 
     else
      {        
