          ...
          <div class="">
            <div class="page-title">
              <div class="title_left">
                <h3>Hybrid Breed <small>species</small></h3>
              </div>
                  ...
                  <div class="x_content"> 
                    <form class="form-horizontal form-label-left input_mask" method="POST"> 
                      ...
                      <div class="col-md-9 col-sm-9 col-xs-12 form-group has-feedback" > 
                       ...
                       <option value="">Select breed three</option>
                       <?php while ($row=$result->fetch_array(MYSQLI_ASSOC)) { ?>
                       <option value=<?php echo $row['id']?>><?php echo $row['name']?></option>
                       <?php } ?>
                       </select> 
...
<?php
  ...
  if(isset($_POST['savecat']))
   { 
    ...
    $count=mysqli_query($con,"SELECT * FROM plaas_hy_breeds WHERE name = '$breedname'");
    if(mysqli_num_rows($count) < 1)
    {  
    $pdo = MyData::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql1 = "INSERT into plaas_hy_breeds(name, categoryid, cross_a, cross_b, cross_c, productiontype, countryoforigin, description, icon) VALUES (?,?,?,?,?,?,?,?,?)";   
    $q1 = $pdo->prepare($sql1);
    $q1->execute(array($breedname,$breedcategory,$breedcrossa,$breedcrossb,$breedcrossc,$breedproduct,$breedcountry,$breedescription,$breedicon));      
    echo '<meta content="2;animal_breed_hybreed_list.php?breed_sucess" http-equiv="refresh" />';// redirects user view page after 3 seconds.
    MyData::disconnect();  
    } 
     else
      {        
      echo '<meta content="2;animal_breed_hybreed_list.php?breed_error" http-equiv="refresh" />';// redirects user view page after 3 seconds. 
      }
   }
?>   
